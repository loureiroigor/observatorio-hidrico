"""gera relatorio de validacao do indice de risco contra classes ana.

para rodar: py -m src.processing.relatorio_ana --modelo "data/templates/dados_modelo.csv" --ana "data/templates/dados_ana.csv" --outdir "reports"
"""
...
def _save_confusion_png(matrix_df: pd.DataFrame, output_png: Path, title: str) -> None:
    ...
    # fallback visual pra nao travar entrega academica se seaborn faltar
...
def gerar_relatorio_validacao_ana(
    ...
    paths = {
        "matriz_csv": output_dir / "relatorio_matriz_confusao_ana.csv",
        "comparativo_csv": output_dir / "relatorio_comparativo_modelo_ana.csv",
        "metricas_csv": output_dir / "relatorio_metricas_ana.csv",
        "matriz_png": output_dir / "matriz_confusao_ana.png",
    }
    ...
    (matrix_df if isinstance(matrix_df, pd.DataFrame) else pd.DataFrame()).to_csv(paths["matriz_csv"], index=bool(isinstance(matrix_df, pd.DataFrame) and not matrix_df.empty))
    if isinstance(matrix_df, pd.DataFrame) and not matrix_df.empty:
        _save_confusion_png(matrix_df, paths["matriz_png"], "matriz de confusao - modelo vs ana")
    if isinstance(comparativo_df, pd.DataFrame):
        comparativo_df.to_csv(paths["comparativo_csv"], index=False)
    metricas_df.to_csv(paths["metricas_csv"], index=False)

    return {
        "metricas": metricas_df,
        "matriz_confusao": matrix_df,
        "comparativo": comparativo_df,
        "arquivos": {name: str(path) for name, path in paths.items()},
    }


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="gera relatorio de validacao do indice de risco contra classes da ana")
...
def main() -> None:
    ...
    for nome, arquivo in resultado["arquivos"].items():
