...
class ImasulAdapter(BaseAdapter):
    ...
    def __init__(self, boletim_pdf_url: str | None = None):
        self.boletim_pdf_url = boletim_pdf_url or (
            "https://www.imasul.ms.gov.br/wp-content/uploads/boletim_hidrologico_ms.pdf"
        )
    ...
    def fetch(self) -> AdapterResult:
        try:
            ...
            mean_level = float(selected["Nivel_m"].mean())
            ...
            fallback = pd.DataFrame(
                [
                    {
                        "Rio": "Sem dado IMASUL",
                        "Bacia": "Paraguai/Parana",
                        "Nivel_m": 2.2,
                    }
                ]
            )
            return self.unavailable(
                str(exc), payload={"table": fallback, "mean_level_m": 2.2, "source_url": self.boletim_pdf_url}
            )
    ...
    def _extract_text(pdf_bytes: bytes) -> str:
        try:
            ...
            raise ImportError(
                "Dependencia ausente: instale pdfplumber para leitura de boletins IMASUL."
            ) from exc
    ...
    def _extract_river_levels(text: str) -> pd.DataFrame:
        ...
        for match in pattern.finditer(text):
            river = re.sub(r"\s+", " ", match.group("rio")).strip().title()
            basin_raw = match.group("bacia").strip().lower()
            basin = "Parana" if "parana" in basin_raw else "Paraguai"
            level = float(match.group("nivel").replace(",", "."))
            rows.append({"Rio": river, "Bacia": basin, "Nivel_m": level})

        if rows:
            return pd.DataFrame(rows).drop_duplicates(subset=["Rio", "Bacia"], keep="last")

        pattern_lines = re.compile(r"([A-Za-z\-\s]{3,35})\s+(\d+[\.,]\d+)")
        for line in text.splitlines():
            if not re.search(r"paraguai|parana|paraná", line, flags=re.IGNORECASE):
                continue
            for match in pattern_lines.finditer(line):
                river = re.sub(r"\s+", " ", match.group(1)).strip().title()
